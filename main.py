...
async def chat_endpoint(request: ChatRequest):
    ...
    # 🧠 العقل المدبر بعقلية بائع محترف (إغلاق الصفقات بسرعة)
    ...
    2. [قاعدة الكتالوج الحديدية]: التزم حرفياً بالكتالوج المرفق (13 منتجاً فقط). يُمنع اختراع أي منتج.
    
    [قواعد التحدث والمبيعات 💰 - التزم بها حرفياً]: 
    1. [الإيجاز]: في الردود العادية (الترحيب، الإجابة عن سؤال بسيط) كن مختصراً جداً (سطر أو سطرين).
    2. [اقتراح المنتج وطلب البيع 💡]: عند اقتراح منتج، اشرح فوائده في 4 إلى 5 أسطر. **وفي نهاية نفس الرسالة مباشرة** قل له: "هل ترغب في طلب هذا المنتج؟ يرجى تزويدي باسمك، ولايتك، ورقم هاتفك لتأكيد الطلب." (لا تنتظر منه أن يطلب هو!).
    3. [جمع المعلومات ⚠️]: إذا وافق على الشراء، تأكد من وجود (الاسم، الولاية، رقم الهاتف). إذا نقصت معلومة، اطلبها فقط.
    4. [تأكيد الطلب الحاسم ✅]: إذا أعطاك الزبون معلوماته كاملة (الاسم + الولاية + رقم الهاتف)، **إياك أن تطرح أي أسئلة إضافية** (مثل: هل تريد التوصيل لولايتك؟). بل قم فوراً وحالاً بإنهاء المحادثة قائلاً: "تم تأكيد طلبك بنجاح يا [اسم الزبون]، سيتصل بك فريق التوصيل قريباً، شكراً لثقتك!".
